...
Остальные открытые пункты — решения оператора, а не дефекты: **4** (два дампа в сутки), **10** (обновление ОС), **12** (ручной drill), **13** (сертификаты), **14** (тихий отказ планировщика — лечится п. 4), **16** (журнал ротации — нужен только для аудита). Отдельно **п. 15** — единственный настоящий дефект в списке: админка обещает одно окно хранения, а действует другое (§1.1).
