...
|---|---|---|---|
| **A1** | PostgreSQL `vertex_ar` | ✅ **Да** — ежедневно 03:00 (APScheduler внутри приложения), `company_id=4`. Дамп шифруется (`age`). ⚠️ `arv-backup-db.timer` **намеренно не установлен** — иначе два дампа в сутки (§10 п. 4) | `python -m app.cli.backup restore <id> --target-db <db>` (нужен ключ — §3) |
...
| Факт | Значение |
|---|---|
| БД | backup **143**, `encrypted=t`, 68 941 Б, `backups/backup_20260915_231904.sql.gz.age` |
...
| `arv-backup-db.timer` | **не установлен намеренно** — иначе два дампа в сутки (см. §10) |
...
| 4 | **Таймер `arv-backup-db` не установлен** | ⏸ решение за оператором | Работает только планировщик внутри приложения: упало приложение — остановились и бэкапы. Но включить таймер при работающем APScheduler = **два дампа в сутки** | Либо оставить APScheduler, либо включить таймер и выключить `backup_enabled` в `system_settings`. CLI теперь умеет работать без `--company-id`, так что таймер работоспособен |
...
| 7 | 🔴 **Второго off-site нет** — `BACKUP_SECONDARY_RCLONE_REMOTE` не задан | ❌ **открыто — главный оставшийся риск** | Бэкап БД уходит на тот же Яндекс Диск, что и прод-медиа; restic-репозиторий (195 МБ) и архив секретов лежат на **том же диске** `/dev/vda2`, что и прод. Одна точка отказа накрывает всё сразу: потеря аккаунта или диска = потеря и данных, и бэкапов | Настроить rclone-remote другого провайдера (B2/Selectel) → `BACKUP_SECONDARY_RCLONE_REMOTE`; вынести `BACKUP_RESTIC_REPOSITORY` на внешний backend (`s3:…`), а не в `/var/backups/arv` |
...
Остальные открытые пункты — решения оператора, а не дефекты: **4** (два дампа в сутки), **10** (обновление ОС), **12** (ручной drill), **13** (сертификаты), **14** (тихий отказ планировщика — лечится п. 4).
...
`BACKUP_STAGING_DIR=/var/backups/arv` · `BACKUP_RESTORE_JOBS=4` · `BACKUP_DRILL_DB_PREFIX=arv_drill` · `BACKUP_MAX_AGE_HOURS=26` · `BACKUP_SECRETS_MAX_AGE_HOURS=192` (секреты недельные — суточный лимит давал бы ложный `STALE` 6 дней из 7) · таймауты: decrypt/restore 3600 с, `--list` 600 с, media-снапшот 6 ч · артефакт БД: `backup_<ts>.sql.gz.age` (внутри — custom-format `-Fc`, восстанавливается **только** через `pg_restore`) · `--trigger` — **top-level** аргумент CLI, до подкоманды · `verify` проверяет **только** тип `db`; медиа проверяется `verify-media` (`restic check`), потому что у медиа-строки нет ни `yd_path`, ни `checksum`.
