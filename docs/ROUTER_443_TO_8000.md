...
## https://ar.neuroimagen.ru на телефоне не открывается

Если в браузере на телефоне **https://ar.neuroimagen.ru** или **https://ar.neuroimagen.ru/api/health** не открываются (таймаут, «нет соединения», «сайт недоступен»), проверьте по шагам:

### 1. DNS
- Домен **ar.neuroimagen.ru** должен указывать на **внешний (белый) IP вашего роутера**.  
- Проверка: на ПК откройте https://whoer.net или https://2ip.ru — запомните «Ваш IP». На телефоне (через мобильный интернет или другой Wi‑Fi) откройте в браузере `https://ВАШ_IP` — если при этом в адресной строке предупреждение о сертификате, но страница грузится, значит порт 443 до роутера доходит. Тогда проблема может быть в DNS (домен не указывает на этот IP).  
- Если используете динамический DNS (No-IP, DuckDNS и т.п.), убедитесь, что он обновлён на текущий IP роутера.

### 2. Проброс порта 443 на роутере
- Должно быть правило: **внешний порт 443** → **IP вашего ПК в локальной сети**, порт **8000**.  
- «Выход» или «Устройство» — тот ПК, где запущен сервер (по имени или по локальному IP, например 192.168.x.x).  
- Без этого правила запросы с телефона на 443 не попадут на ПК.

### 3. Брандмауэр Windows
- Разрешите **входящие** подключения по **TCP, порт 8000** для приложения Python/uvicorn (или для «частной» сети).  
- Как проверить: временно отключите брандмауэр и с телефона снова откройте https://ar.neuroimagen.ru — если заработало, настройте правило для порта 8000 и включите брандмауэр обратно.

### 4. Сервер запущен с SSL
- На ПК должен быть запущен `python scripts/run_server.py` (или `python -m app.main`), в `.env` заданы `SSL_KEYFILE` и `SSL_CERTFILE`.  
- В логах при старте должно быть: `Uvicorn running on https://0.0.0.0:8000`.

### 5. Телефон и сеть
- Проверьте с телефона в браузере именно **https://ar.neuroimagen.ru** (не http и не с :8000).  
- Если телефон в **той же Wi‑Fi**, что и ПК, часть роутеров разрешает обращаться к внешнему IP изнутри — тогда должно открываться. Если не открывается даже с этого Wi‑Fi, чаще всего причина в пробросе 443 или брандмауэре.
