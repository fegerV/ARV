...
4. [Проблемы с AR-маркерами](#проблемы-с-ar-маркерами)
...
**Доступ к ar.neuroimagen.ru** — без тоннеля: используется прямой проброс порта на роутере (443→8000 или 8000→8000). Тоннель (ngrok) в текущей схеме не используется.
...
AR-просмотр реализован через Android-приложение «AR Viewer» (ARCore). Веб-AR (MindAR) отключён. Ссылка `/view/{unique_id}` ведёт на лендинг с кнопками «Открыть в приложении» и «Скачать в Google Play».
...
## Проблемы с AR-маркерами

Маркер для ARCore = загруженное фото (JPEG/PNG). Генерация .mind не требуется.

**Рекомендации для хорошего трекинга:**
- Изображение с высоким контрастом и чёткими границами
- Минимум 640×480, рекомендуется 1920×1080+
- Формат: JPEG или PNG
