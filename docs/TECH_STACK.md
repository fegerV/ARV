...
- WebSocket (частично реализован через `alerts_ws.py`)
...
- Серверный рендеринг
- Поддержка WebSocket (частично реализован через `alerts_ws.py`)
...
- Проверка паролей
- Legacy fallback: unsalted SHA-256 для обратной совместимости

### bcrypt 4.3.0

**Описание**: Алгоритм хеширования паролей

**Примечание**: Доступен как дополнительная схема в passlib, но основной используемый алгоритм — pbkdf2_sha256.
