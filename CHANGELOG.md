...
- **CLI** `app/cli/backup.py`: `db`, `media`, `secrets`, `verify`, `verify-media`,
  `drill`, `restore`, `download`, `status`, `notify`. (Подкоманд `run` и `list` нет —
  запуск делают `db`/`media`/`secrets`, а список отдаёт `restore.sh --list`.)
...
- **Руководство по бэкапам в справке админки.** Раздел «Резервные копии и восстановление»
  (`templates/help/partials/backups_section.html`) описывает, что попадает в копии, как
  настроить расписание и лестницу хранения, как создать копию вручную и как проверить,
  что она пригодна. Операторская часть — восстановление из копии, включая путь через
  `--from-file`, когда БД потеряна, — показывается только супер-админам: остальным
  выводится пояснение, что восстановление выполняет администратор. Причина простая:
  `/backups` и вкладка настроек и так супер-админские, а пути хоста и CLI-команды
  в справке, доступной любому тенанту, — лишняя выдача.
...
- **`/api/backups/*` больше не доступен обычному пользователю.** Четыре эндпоинта
  (`run`, `history`, `status`, `DELETE /{id}`) принимали любого аутентифицированного и
  скоупились по `backup_history.company_id` — колонке «чей Яндекс.Диск хранит артефакт»,
  а не «чей тенант владеет данными». Пользователи компании-хранилища могли запускать,
  перечислять и удалять бэкапы платформы, хотя `/backups` и вкладка настроек, стоящие
  перед тем же API, были супер-админскими. Теперь все пять эндпоинтов требуют
  `require_super_admin` (как и `download`); регрессия закреплена
  `test_backup_endpoints_reject_non_super_admin`.
