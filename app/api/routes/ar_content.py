...
@router.post("/api/ar-content", response_model=ARContentCreateResponse, tags=["AR Content"])
...
async def create_ar_content_for_project(
    company_id: int,
    project_id: int,
    customer_name: Optional[str] = Form(None),
    customer_phone: Optional[str] = Form(None),
    ...
    """Create new AR content with photo and video files for a specific company and project."""
    # Validate company and project relationship
    company, project = await validate_company_project(company_id, project_id, db)
...
@router.get("/api/ar-content/{id}", response_model=ARContentWithLinks, tags=["AR Content"])
async def get_ar_content(
    ...
    # Get AR content
    ar_content = await get_ar_content_or_404(id, db)
    
    # Load related videos
    stmt = select(Video).where(Video.ar_content_id == id)
    result = await db.execute(stmt)
